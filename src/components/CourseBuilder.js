  ...
  switch (action.type) {
    ...
    case "DELETE_LESSON":
      return {
        ...state,
        lessonCount: state.lessonCount - 1,
        lessons: state.lessons.filter(({ lessonId }) => lessonId !== action.lessonId)
      }
    default:
  ...
  return (
    <div className="course-builder">
      ...
      <LessonBuilder
        ...
        onAdd={(lesson) => dispatch({ type: "ADD_LESSON", lesson })}
        onRemove={(lessonId) =>
          dispatch({ type: "DELETE_LESSON", lessonId })
        }
      />
      <button className="course-action-btn" onClick={() => (state.courseTitle ? onCreate(state) : null)}>
        Done
      </button>
